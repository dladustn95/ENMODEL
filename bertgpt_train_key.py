...
def build_input_from_segments(source, target, score, tokenizer, lm_labels=False, with_eos=True):
    ...
    instance = {}
    instance["source_ids"] = [101] + source
...
def train():
    ...
    if args.model_checkpoint != "":
        ...
        gpt_config = config_class.from_pretrained("gpt2")
    ...
    if args.model_checkpoint != "":
        ...
        model = model_class.from_pretrained("gpt2", config=gpt_config)
    ...
    def update(engine, batch):
        ...
        if args.keyword_module == "attention":
        ...
        elif args.keyword_module == "new":
            encoded_layers, _ = bert_model(source_ids)

            encoded_layers_key = encoded_layers
            key_score = keyword_scores.tolist()
            for i, score in enumerate(key_score):
                key_score_list = list(filter(lambda a: a != tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[-1]), score))
                if len(key_score_list) != 1:
                    ...
                    key_score_list = list(map(lambda a: a, key_score_list))
                else:
                    key_score_list = [1]
            ...
            (lm_loss), *_ = model(target_ids, encoded_layers, attention_score=encoded_layers_key, labels=lm_labels)
        elif args.keyword_module == "new0.5":
            encoded_layers, _ = bert_model(source_ids)
            ...
            for i, score in enumerate(key_score):
                ...
                if len(key_score_list) != 1:
                    key_score_list = list(map(lambda a: a - min(key_score_list), key_score_list))
                    key_score_list = list(map(lambda a: a / max(key_score_list), key_score_list))
                    key_score_list = list(map(lambda a: a*2, key_score_list))
                else:
                    key_score_list = [1]
        ...
        with torch.no_grad():
            ...
            if args.keyword_module == "attention":
                ...
                for i, score in enumerate(key_score):
                    ...
                    for k, v in enumerate(tmp):
                        encoded_layers_key[i][k+1] = encoded_layers_key[i][k+1] * v
                lm_logits, *_ = model(target_ids, encoded_layers, attention_score=encoded_layers_key)
                ...
                for i, score in enumerate(key_score):
                    key_score_list = list(
                        filter(lambda a: a != tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[-1]), score))
                    if len(key_score_list) != 1:
                        key_score_list = list(map(lambda a: a - min(key_score_list), key_score_list))
                        key_score_list = list(map(lambda a: a / max(key_score_list), key_score_list))
                        key_score_list = list(map(lambda a: a + 0.5, key_score_list))
    ...
    def custom_event_filter(engine, event):
        if event==engine.state.iteration:
            return True
        return False
