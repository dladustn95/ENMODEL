...
from transformers import (AdamW, GPT2Config, GPT2Tokenizer, WEIGHTS_NAME, CONFIG_NAME, BertTokenizer, BertModel)
...
def train():
    ...
    parser.add_argument("--gpt2_model_name", type=str, default="bertTran", help="Path, url or short name of the model")
    ...
    if args.model_checkpoint != "":
        ...
        tran_config = GPT2Config()
        tran_config.keyword_module = args.keyword_module
        tran_config.n_layer = 6

    if args.model_checkpoint != "":
        model = Decoder.from_pretrained(args.model_checkpoint, config=tran_config)
    else:
        model = Decoder(tran_config)
