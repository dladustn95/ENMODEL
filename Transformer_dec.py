...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False, keyword_module=""):
        super().__init__()
        nx = config.n_embd
    ...
    def forward(self,  query, key, value, key_key=None, key_value=None, layer_past=None, attention_mask=None, head_mask=None):
        output_attn = self.attn(
            query, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask
        )
        a = output_attn[0]  # output_attn: a, present, (attentions)
        x = self.ln_1(query + a)

        output_attn = self.multihead_attn(
            x, key, value, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask
        )
        a = output_attn[0]  # output_attn: a, present, (attentions)
        x = self.ln_3(x + a)
        ...
        if self.keyword_module != "":
            output_attn = self.multihead_attn2(
                x, key_key, key_value, layer_past=layer_past, attention_mask=attention_mask,
                head_mask=head_mask
            )
            a = output_attn[0]  # output_attn: a, present, (attentions)
            x = self.ln_4(x + a)
...
class TransformerDecoder(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([Block(config.n_ctx, config, scale=True, keyword_module=self.keyword_module) for _ in range(config.n_layer)])
...
class Decoder(GPT2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
